...
vignette: >
  %\VignetteIndexEntry{vocabulary_ad}
...
`factor_levels`: vector of value labels  
`n_categories`: total number of response options `= length(factor_levels)`   
`class_suggested`: suggested conversion to format  
`filename`: original name of file as obtained from GESIS  
...
`val_order_alpha`: number (position) of response option in the set of value labels  
`val_order_length`: **what's this, and why is it equal to `n_categories` - 1?**  
`val_label_norm`: value label normalized
