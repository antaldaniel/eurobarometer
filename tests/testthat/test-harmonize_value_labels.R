...
test_that("binary harmonization works", {
  expect_equal(as.numeric ( harmonize_value_labels(v, 2) ),
               c(1,0,0, 1, 8999,9998))
...
test_that("three value harmonization works", {
  expect_equal(as.numeric ( harmonize_value_labels(labelled_var = v3,categories = 3) ),
               c(1,-1,0, 8999,9998))
  expect_equal(labelled::to_character ( harmonize_value_labels(v3,3) ),
               c("better", "worse", "same", "wrong_label", "declined"))
})
