...
code_nuts2 <- function ( country_code = NULL,
 ...
 utils::data("vocabulary_nuts2", package = "eurobarometer", envir = environment())

 nuts2 <- vector ( mode = "character", length = length(region_nuts_names))

 if ( is.null(country_code) ) {
   vocabulary <- vocabulary_nuts2 %>%
   ...
   nuts2_df <- dplyr::left_join ( df, vocabulary,
     ...
     filter ( !is.na(region_nuts_names)) %>%
 ...
 if (nuts_code == "code10")  return( nuts2_df$code10 )
 if (nuts_code == "code13")  return( nuts2_df$code13 )
