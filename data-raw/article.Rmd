...
The consistent, harmonized treatment of the missing values allows us to easily implement the [as_numeric, as_factor and as_character](http://retroharmonize.satellitereport.com/reference/labelled_spss_survey.html) methods, by replacing the user-defined missing values with type-specific `NA` values in the R language (`NA_real_` or `NA_character_`). After this conversion, R artithmetic and statistical functions can be applied without compromising reproducability --- all metadata is present for a consistent export to SPSS or STATA, or to document, or even revert to an earlier labelling.
